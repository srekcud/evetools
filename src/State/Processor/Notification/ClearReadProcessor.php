 ...
 */
class ClearReadProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): NotificationResource
    {
        ...
        $deleted = $this->notificationRepository->deleteReadForUser($user);

        $resource = new NotificationResource();
        $resource->id = 'clear-read';
        $resource->category = '';
        $resource->level = 'info';
        $resource->title = 'Read notifications cleared';
        $resource->message = sprintf('%d read notifications deleted', $deleted);
