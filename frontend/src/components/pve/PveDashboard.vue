...
<template>
  <div class="space-y-6">
    ...
    <div v-if="pveStore.isLoading" class="flex items-center justify-center py-12">
      ...
      <span class="ml-3 text-slate-400">{{ t('common.status.loading') }}</span>
    </div>
    ...
    <template v-else>
      ...
      <div v-if="pveStore.stats" class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-slate-800 rounded-lg p-4">
          <div class="text-slate-400 text-sm">{{ t('ledger.kpi.totalIncome') }}</div>
          <div class="text-2xl font-bold text-emerald-400">
            {{ formatIsk(pveStore.stats.totals.income) }}
          </div>
        </div>
        <div class="bg-slate-800 rounded-lg p-4">
          <div class="text-slate-400 text-sm">{{ t('ledger.kpi.totalExpenses') }}</div>
          <div class="text-2xl font-bold text-red-400">
            {{ formatIsk(pveStore.stats.totals.expenses) }}
          </div>
        </div>
        <div class="bg-slate-800 rounded-lg p-4">
          <div class="text-slate-400 text-sm">{{ t('ledger.kpi.netProfit') }}</div>
          <div
            :class="[
              'text-2xl font-bold',
        ...
        <div class="bg-slate-800 rounded-lg p-4">
          <div class="text-slate-400 text-sm">{{ t('ledger.kpi.dailyAverage') }}</div>
      ...
      <div v-if="pveStore.stats" class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-slate-900 rounded-lg p-3 border border-slate-800">
          <div class="text-slate-500 text-xs">{{ t('pve.lootSales') }}</div>
      ...
      >
        ...
        <p class="text-lg">{{ t('pve.noDataToDisplay') }}</p>
        <p class="text-sm mt-1">{{ t('pve.startTrackingHint') }}</p>
      </div>
    </template>
  </div>
</template>
